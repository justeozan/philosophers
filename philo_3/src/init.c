...
/*   Updated: 2024/06/18 16:02:06 by ozasahin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_calloc2(void **ptr, size_t size, size_t __nmemb)
{
	*ptr = calloc(__nmemb, size);
	if (!*ptr)
		return (write(2, ERR_MALLOC, 24), 1);
		// return (printf(law, ERR_MALLOC, ));
	return (0);
...
void	init_mutex(t_law *law, int nbr_philos)
{
	int	i;

	mutex_handle(&law->write_lock, INIT);
	mutex_handle(&law->law_mutex, INIT);
	memset(law->forks, 0, nbr_philos * sizeof(int));
	i = -1;
	while (++i < nbr_philos)
...
bool	initialize_data(t_law *law, int nb_philo, char **av)
{
	initialize_ptr_to_null(law);
	if (ft_calloc2((void **)&law, 1, sizeof(t_law *))
		|| ft_calloc2((void **)law->forks_mtx, nb_philo, sizeof(t_mutex))
		|| ft_calloc2((void **)law->forks, nb_philo, sizeof(int)))
	...
	// 	return (free_data(law), false);
	init_philos(law->philos, law, law->forks);
	init_mutex(law, law->nbr_philos);
	return (true);
}
